...
%RUN this MATLAB script and analyze each one of the functions used to implement
%the algorithm described in the Assignment Doc.

%Learn how to DEBUG the code, by checking the latent variables of the
%code and showing the intermediate stages of the pipeline.

% load the image
raw = imread('../data/banana_slug.tiff');
%raw = imread('DCS00080.tiff');
...
red = im_ww(1:2:end, 1:2:end);
green1 = im_ww(1:2:end, 2:2:end);
green2 = im_ww(2:2:end, 1:2:end);
blue = im_ww(2:2:end, 2:2:end);


im_rgb_Int = cat(3, red, (green1+green2)/2, blue);
